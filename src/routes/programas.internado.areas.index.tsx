...
export const Route = createFileRoute("/programas/internado/areas/")({
  head: () => ({
    meta: [
      { title: "Rotaciones del Internado Médico · KotaMed" },
      {
        name: "description",
        content:
          "Los módulos del Internado Médico: Medicina Interna, Cirugía General, Gineco-Obstetricia, Pediatría & Neonatología, Emergencias y Salud Comunitaria.",
      },
      { property: "og:title", content: "Rotaciones del Internado Médico · KotaMed" },
      {
        property: "og:description",
        content: "Cada rotación es un ecosistema de aprendizaje independiente y editable.",
      },
  ...
  loader: () => {
    if (INTERNADO_AREAS.length === 0) throw notFound();
  },
  component: InternadoAreasHub,
});

function InternadoAreasHub() {
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-10 py-12 relative">
        <div className="flex items-center gap-3 mb-6">
          <Link
            to="/programas/$slug"
            params={{ slug: "internado" }}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground transition"
          >
            <ArrowLeft className="size-3.5" /> Internado Médico
          </Link>
          <span className="text-muted-foreground/40">/</span>
          <span className="text-xs font-semibold text-foreground">Rotaciones</span>
        </div>
        ...
        <section className="glass rounded-3xl p-8 md:p-10 animate-slide-up">
          <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border border-primary/30 bg-primary/10 text-primary">
            ...
            {INTERNADO_AREAS.length} rotaciones hospitalarias
          </span>
          <h1 className="mt-4 text-4xl md:text-5xl font-extrabold tracking-tight leading-[1.05]">
            Módulos del Internado Médico
          </h1>
          <p className="mt-4 text-base md:text-lg text-foreground/80 max-w-3xl">
            Cada rotación funciona como una plataforma independiente: landing propia, ruta
            académica, contenido editable por el admin, casos clínicos, banco de preguntas,
            flashcards, simuladores, biblioteca y tutor IA con evidencia citada.
          </p>
        </section>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mt-8">
          {INTERNADO_AREAS.map((a, i) => {
            const Icon = a.icon;
            return (
              <Link
                key={a.slug}
